...
class Sender {
  public:
    static core::Task<core::Expected<>> send(net::details::Stream &stream,
        ...
        if (res.type == ResponseType::kFile) {
            ...
            co_await co_await stream.write(res.to_head_string());
  ...
  private:
    static core::Task<core::Expected<>> send_file(net::details::Stream &stream,
        ...
        if (size == 0) {
            // mmap 长度为 0 会直接 EINVAL。空文件回一个空 body 就行
            ::close(fd);
...

        {
            ...
            if (!file_set.contains(file_path)) {
                file_set.try_emplace(
                    file_path,
                    std::make_pair(
                        std::shared_ptr<void>(
                            mapped, [size](void *p) { ::munmap(p, size); }),
                        size));
            } else {
                ::munmap(mapped, size);
            }
    ...
    /// 单次写出的分片大小。原来引用的是 Stream::kChunkSize，那个常量
    /// 全项目零引用，已经删掉了
    ...
    mmap_write(net::details::Stream &stream,
               Response &res,
               std::shared_ptr<void> ptr_holder,
               size_t total_size) {
        ...
        while (remaining > 0) {
            size_t to_send = std::min(remaining, kSendChunk);
